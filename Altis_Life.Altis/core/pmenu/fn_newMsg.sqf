...
switch(_type) do{ 
	...
	//normal message 
	case 1: { 
		if(isNUll life_smartphoneTarget) exitWith {hint format["Kein Empfänger ausgewählt!"];};
	...
	//copmessage 
	case 2: { 
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["Die Polizei ist leider momentan nicht erreichbar. Versuch es später nochmal."];};
	...
	//emsrequest 
	case 4: { 
		if(({side _x == independent} count playableUnits) == 0) exitWith {hint format["Die Sanitäter sind leider momentan nicht erreichbar. Versuch es später nochmal."];};
		ctrlShow[888899,false];
		if(_msg == "") exitWith {hint "Du musst eine Nachricht eintippen zum Senden!";
			ctrlShow[888899,true];
		};
		[ObjNull,_msg,player,3] remoteExec ["TON_fnc_handleMessages",2];
		hint format["Du hast eine Nachricht an alle Sanitäter geschickt.",_msg];
		ctrlShow[888899,true];
		closeDialog 887890;
	};
	//adminToPerson 
